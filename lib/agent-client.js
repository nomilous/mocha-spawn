...
AgentClient.prototype.stopProcess = stopProcess;
...
function spawnProcess(id, run, opts) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    debug('start %s (%s) at %s', run.script, id, _this.hostname);
    ...
    _this.socket.once(proc.id, function (result) {
      switch (result.action) {
        ...
        case 'started':
          ...
          debug('started %s (%s) at %s', run.script, id, _this.hostname);
          proc.state = 'running';
        ...
        case 'startError':
          ...
          debug('start error %s (%s) at %s', run.script, id, _this.hostname);
          var e = new Error();
          e.name = result.name;
...
function stopProcess(id, run, opts) {
  var _this = this;
  return new Promise(function (resolve, reject) {
    ...
    debug('stop %s (%s) at %s', run.script, id, _this.hostname);
    ...
    _this.socket.once(proc.id, function (result) {
      switch (result.action) {
        ...
        case 'stopped':
          debug('stopped %s (%s) at %s', run.script, id, _this.hostname);
          deleteProcess();
          return resolve();

        case 'stopError':
          debug('stop error %s (%s) at %s', run.script, id, _this.hostname);
          var e = new Error();
          e.name = result.name;
          e.message = result.message;
          // e.stack = result.stack;
