...
AgentServer.prototype._doProcessStop = _doProcessStop;
...
function _doProcessStart(socket, msg) {
  debug('start %s (%s)', msg.run.script, msg.id);
  var _this = this;
  var proc;
  var child;
  var script;
  ...
  child.on('message', function (message) {
    ...
    switch (message.action) {
      ...
      case 'startError':
        ...
        child.removeListener('exit', onExit);
        _this._doProcesskill(socket, proc, true);
        socket.emit(proc.id, {
          action: 'startError',
          id: proc.id,
          name: message.name,
...
function _doProcessStop(socket, msg) {
  var _this = this;
  ...
  proc._child.on('exit', onExit);

  proc._child.on('message', function (message) {
    ...
    switch (message.action) {
      ...
      case 'stopError':
        ...
        proc._child.removeListener('exit', onExit);
        _this._doProcesskill(socket, msg, true);
...
function _doProcesskill(socket, msg, skipResponse) {
  console.log('do process kill');
  var proc = socket.procs.find(function (proc) {
    return proc.id == msg.id;
  });
...
function _killAllProcesses(socket) {
  var proc;
  for (var i = socket.procs.length - 1; i >= 0; i--) {
    this._doProcesskill(socket, socket.procs[i], true);
  }
}
